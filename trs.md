...
Two built-in string transforms are available: `"filter_markdown"` strips Markdown formatting and `"filter_emoji"` removes emoji characters. When `tts_text_transforms` is not set, both are applied by default. Set it to `None` to turn off all transforms.
...
The `replace()` function accepts a `replacements` dictionary mapping source terms to their replacements, and an optional `case_sensitive` flag (default: `False`).
...
You can pass any callable with the signature `Callable[[AsyncIterable[str]], AsyncIterable[str]]` as a transform. This lets you implement custom text processing logic that operates on the streaming text.
...
Transforms are applied in order. In the example above, Markdown is stripped first, then emails are redacted, then text replacements are applied.
...
This document was rendered at 2026-04-05T06:22:13.612Z.
