...
export class DecisionEngine {
  ...
  shouldSuggest(input: DecisionInput): boolean {
    ...
    if (
      ...
      (input.hasTranscriptActivity || input.hasRecentChatActivity) &&
      input.changeCount >= thresholds.minChanges &&
      input.timeSinceLastChange >= thresholds.silenceThreshold
    ) {
      return true;
  ...
  shouldAct(input: DecisionInput): boolean {
    return this.shouldSuggest(input);
  }

  allowsDraftActions(): boolean {
    return this.intensity !== "quiet";
  }
}
