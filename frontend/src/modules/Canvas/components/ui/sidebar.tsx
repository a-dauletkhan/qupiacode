...
import { Separator } from "@/modules/Canvas/components/ui/separator"
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
} from "@/modules/Canvas/components/ui/sheet"
import { Skeleton } from "@/modules/Canvas/components/ui/skeleton"
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@/modules/Canvas/components/ui/tooltip"
import { PanelLeftIcon } from "lucide-react"

const SIDEBAR_COOKIE_NAME = "sidebar_state"
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * 7
const SIDEBAR_WIDTH_DEFAULT = 25
const SIDEBAR_WIDTH_MIN = 20
...
function SidebarRail({
  ...
  const { isMobile, setSidebarWidth, state, toggleSidebar } = useSidebar()
  ...
  const handlePointerDown = React.useCallback(
    (event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      const sidebarSide = event.currentTarget
      ...
      const handlePointerMove = (moveEvent: PointerEvent) => {
        ...
        setSidebarWidth(
          Math.min(
            SIDEBAR_WIDTH_MAX,
            Math.max(SIDEBAR_WIDTH_MIN, nextWidth)
          )
        )
      }
